...
function M.add_worktree(on_success_callback)
    ...
    vim.ui.input({ prompt = "Enter new branch/worktree name: " }, function(branch_name)
        ...
        if vim.fn.isdirectory(absolute_worktree_path) == 1 then
            vim.notify(string.format("Worktree directory '%s' already exists.", absolute_worktree_path), vim.log.levels.ERROR)
            return
        end
        ...
        if branch_exists_on_remote then
            ...
            -- Use the absolute path for the worktree location
            worktree_cmd = string.format("git -C '%s' worktree add '%s' '%s'", repo_root, absolute_worktree_path, branch_name)
        else
            vim.notify(string.format("Branch '%s' not found. Creating new local branch and worktree...", branch_name))
            -- Use the absolute path for the worktree location and the -b option
            worktree_cmd = string.format("git -C '%s' worktree add -b '%s' '%s'", repo_root, branch_name, absolute_worktree_path)
        end
